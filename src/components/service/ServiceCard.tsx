...
const ServiceCard: React.FC<ServiceCardProps> = ({ service, onEdit, onDelete }) => {
    ...
    const [pendingValues, setPendingValues] = useState<Partial<Service> | null>(null);
                    ...
                    {pendingValues && (
                        <div className="mt-4 p-4 bg-gray-50 rounded">
                            ...
                            <ul className="list-disc pl-5">
                                {pendingValues.title && pendingValues.title !== service.title && (
                                    <li>
                                        <span className="font-medium">Title:</span> {pendingValues.title}
                                    </li>
                                )}
                                {pendingValues.subtitle && pendingValues.subtitle !== service.subtitle && (
                                    <li>
                                        <span className="font-medium">Subtitle:</span> {pendingValues.subtitle}
                                    </li>
                                )}
                                {pendingValues.descriptions && pendingValues.descriptions !== service.descriptions && (
                                    <li>
                                        <span className="font-medium">Description:</span> {pendingValues.descriptions.substring(0, 50)}...
                                    </li>
                                )}
                                {pendingValues.icon && pendingValues.icon !== service.icon && (
                                    <li>
                                        <span className="font-medium">Icon:</span> {pendingValues.icon}
                                    </li>
                                )}
                            </ul>
                        </div>
